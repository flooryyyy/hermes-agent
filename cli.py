...
class HermesCLI:
    ...
    def _handle_pipeline_command(self):
        """Show model training pipeline status."""
        from hermes_cli.pipeline_status import gather_pipeline_data

        data = gather_pipeline_data()

        lines = []
        # Section 1: Current model
        lines.append("── Pipeline Status ──")
        lines.append(f"  Model: {data['current_model']}")
        lines.append(f"  Server: {data['llama_swap_host']}:{data['llama_swap_port']}")
        lines.append("")

        # Section 2: Benchmarks
        b = data["benchmarks"]
        if b["file_found"]:
            lines.append("── Benchmarks (v3) ──")
            lines.append(f"  IFEval prompt_strict: {b.get('ifeval_prompt_strict', '?')}  "
                         f"(vs v2: {b.get('ifeval_vs_v2', '?')}, vs base: {b.get('ifeval_vs_baseline', '?')})")
            lines.append(f"  Style composite:     {b.get('style_composite', '?')}  "
                         f"(vs v2: {b.get('style_vs_v2', '?')}, vs base: {b.get('style_vs_baseline', '?')})")
        else:
        ...
        if k["available"]:
            if k["tasks"]:
                for t in k["tasks"]:
                    lines.append(f"  [{t['status']:12s}] {t['id']} — {t['title']} ({t['assignee']})")
            else:
                lines.append("  No active tasks")
        else:
            err = k.get("error") or "unavailable"
            lines.append(f"  Kanban: {err}")

        # Section 4: Desktop disk
        d = data["desktop_disk"]
        lines.append("")
        lines.append("── Desktop Disk (/var/lib/llm-models/) ──")
        if d["reachable"]:
            lines.append(f"  {d['used_gb']}G / {d['total_gb']}G ({d['usage_percent']}%)")
        else:
            lines.append("  Desktop not reachable via SSH")

        # Section 5: Pending steps
        lines.append("")
        lines.append("── Next Pipeline Steps ──")
        if data["pending_steps"]:
            for s in data["pending_steps"]:
                lines.append(f"  → {s}")
        else:
            lines.append("  No steps pending")

        # Section 6: Session overview (kanban + pending topics)
        try:
            from hermes_cli.status_summary import (
                _read_kanban_done_today, _read_pending_topics, _running_processes,
            )
            done = _read_kanban_done_today()
            ...
            if done:
                for t in done:
                    lines.append(f"  {t['time']} — {t['title'][:65]} ({t['assignee']})")
            else:
